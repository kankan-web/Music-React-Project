...
export default memo(function SongInfo(props) {
  // const {currentSong,lyricList} = props
  //redux hook
  const {currentSong,lyricList,totalComment} = useSelector(
    (state)=>({
      currentSong:state.getIn(['song','showCurrentSongDetail']),
      lyricList:state.getIn(['song','showCurrentSongLyricList']),
      totalComment: state.getIn(['song', 'showCurrentSongCommentTotal']),
    })
  )
  //other
        ...
        <Collapse>
          <Panel header={`歌词展示`}>
            {lyricList &&
              lyricList.map((item) => {
                return (
                  <div key={item.time} className="lyric-item">
                    {item.content}
                  </div>
                )
              })}
