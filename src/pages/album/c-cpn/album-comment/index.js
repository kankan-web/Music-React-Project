...
export default memo(function AlbumComment() {
  // props/state
  const [songComment, setSongComment] = useState([])//歌曲评论
  const [hotComments,setHotComments] = useState([])//热门评论
  const [action,setAction] = useState(null)
  const [currentPage, setCurrentPage] = useState(1)//当前页码
  const [total, setTotal] = useState(0)
  //redux handle
  const {currentSongId} = useSelector((state)=>({
    currentSongId:state.getIn(['album','albumId']),
  }),shallowEqual)
  const dispatch = useDispatch()

  //other hooks
  useEffect(()=>{
    getAlbumComment(currentSongId).then((res)=>{
  ...
  const changePage = useCallback(
    (currentPage) => {
      ...
      getAlbumComment(currentSongId, 20, targePageCount).then((res) => {
        setSongComment(res.comments)
        setTotal(res.total)
      })
    },
